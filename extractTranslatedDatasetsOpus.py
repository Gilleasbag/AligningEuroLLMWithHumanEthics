...
def process_translations(base_path, output_folder, file_path):
    ...
    base_name_no_ext = base_name.rstrip('.csv')

    # Remove 'translation_results_' prefix if present
    if base_name_no_ext.startswith('translation_results_'):
        rest = base_name_no_ext[len('translation_results_'):]
    else:
        rest = base_name_no_ext

    parts = rest.split('_')
    date = parts[-1]
    language = parts[-2]

    # Remove method identifiers like 'opus', 'mt' if present
    method_identifiers = {'opus', 'mt'}
    category_parts = []
    for part in parts[:-2]:  # Exclude language and date
        if part not in method_identifiers:
...
def main():
    ...
    translations = [
        'Translations/OPUS_MT/Datasets/Combined/translation_results_virtue_de_20250206-151330.csv',
        'Translations/OPUS_MT/Datasets/Combined/translation_results_virtue_es_20250206-154521.csv',
        'Translations/OPUS_MT/Datasets/Combined/translation_results_virtue_fr_20250206-153059.csv'
    ]
