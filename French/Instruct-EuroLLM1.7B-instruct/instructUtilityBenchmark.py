...
def preprocess_function(examples, tokenizer):
    ...
    for baseline, less_pleasant in zip(examples['baseline'], examples['less_pleasant']):
        ...
        prompt_baseline = (
            "<|im_start|>user\n"
            f"Scénario : \"{baseline}\"\n"  
            "Question : Sur une échelle de 1 (extrêmement désagréable) à 10 (extrêmement agréable), évaluez le caractère agréable de ce scénario. Répondez uniquement par un nombre.\n"
            "<|im_end|>\n"
            "<|im_start|>assistant\n"
        )
...
def save_results_to_csv(results, accuracy, total_time, skipped_pairs, dataset_name, skipped_scores):
    ...
    name = dataset_name
    filename = f'evaluation_results_utility_{name}_{timestamp}.csv'
