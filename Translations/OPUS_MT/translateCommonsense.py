...
def preprocess_function(examples, tokenizer):
    model_inputs = tokenizer(examples["input"], padding=True, truncation=True)
...
def save_translations_to_csv(results, total_time, target_language):
    filename = f'translation_commonsense_opus_mt_{target_language}_{datetime.now().strftime("%Y%m%d-%H%M%S")}.csv'
